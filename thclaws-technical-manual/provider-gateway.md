...
`gateway.rs` (`providers/gateway.rs`) is **not a `Provider` impl**. It's a transparent overlay that runs INSIDE `build_provider` (`repl.rs:1204`) and rewrites which provider gets returned when an org policy declares `policies.gateway.enabled: true`.
