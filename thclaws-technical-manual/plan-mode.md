...
Distinct from **TodoWrite** (casual scratchpad, no enforcement, invisible until the user opens `.thclaws/state/todos.md`) and from **subagent** / **agent team** (different process / recursion models). Plan mode is the **highest-ceremony** planning tool: sequential gate, audit-required note on Failed transitions, sidebar visibility, four user override buttons.
