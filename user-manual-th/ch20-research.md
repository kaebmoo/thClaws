...
| `--max-notes N` | 30 | เพดาน note ต่อ run รวมหน้า topic entity ที่บางจะถูกรวมเข้ากับหน้าแม่แทนการตัดทิ้ง **จาก slash command ค่าที่รับได้คือ 1–20 เท่านั้น** ส่วนค่า default 30 และเพดานจริงของ pipeline ที่ 50 จะได้ก็ต่อเมื่อไม่ใส่ flag นี้ |
...
`--max-pages` รับเป็นชื่อแทนของ `--max-notes` ช่วงค่าเดียวกันคือ 1–20

> **ค่า flag ที่ถูกปฏิเสธจะกลายเป็นส่วนหนึ่งของ query** ตัว parser กิน flag
> จากหัวไปเรื่อย ๆ แล้วหยุดที่ token แรกที่ไม่รู้จัก เพื่อให้
> `/research --opinion of the user` ค้นวลีนั้นได้จริงแทนที่จะ error
> ผลข้างเคียงคือ flag ที่ *รู้จัก* แต่ใส่ค่านอกช่วงหรือค่าที่ parse ไม่ได้ —
> `--max-notes 30`, `--novelty 5`, `--max-iter abc` — ก็ไม่ error เช่นกัน
> การ parse จะหยุดตรงนั้นและข้อความของ flag จะถูกเอาไปต่อหน้า query
> ถ้า run กลับมาพร้อมหน้า topic ที่พูดถึง flag ของคุณเอง แปลว่าเจอเคสนี้
> ให้ดู query ที่ echo ออกมาในบรรทัด `[research started: …]`
